          ...
          >
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {album.images.map((src, idx) => (
                <div key={idx} className="overflow-hidden rounded-xl border border-border bg-secondary">
                  <img src={src} alt={`${album.title} ${idx + 1}`} loading="lazy" className="h-full w-full object-cover" />
                </div>
              ))}
            </div>
          </div>
